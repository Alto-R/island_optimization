"""
Advanced Energy System Investment Analysis with Progressive Modeling Approach

This script implements a comprehensive four-stage analysis:
...
3. Core Analysis: XGBoost/Random Forest for high-precision modeling
4. Deep Interpretation: Feature importance and PDP analysis
...
try:
    ...
    print("Warning: XGBoost not available. Will use Random Forest only.")
...
class AdvancedEnergyAnalysis:
            ...
            for idx, island in islands_df.iterrows():
                ...
                if windspeed_file and output_file and capacity_file and cost_data is not None:
                    try:
                        ...
                        # Process cost data
                        renewable_cost = cost_data.get('renewable_cost', 0)
                        electrical_storage_cost = cost_data.get('electrical_storage_cost', 0)
                        thermal_storage_cost = cost_data.get('thermal_storage_cost', 0)
                        lng_cost = cost_data.get('lng_cost', 0)
                        pem_cost = cost_data.get('pem_cost', 0)
                        fc_cost = cost_data.get('fc_cost', 0)
                        
                        total_storage_investment = electrical_storage_cost + thermal_storage_cost + pem_cost + fc_cost
    ...
    def stage3_advanced_modeling(self):
        """
        Stage 3: Core Analysis - XGBoost/Random Forest for high-precision modeling
        """
        print("\n" + "="*80)
        print("🚀 STAGE 3: ADVANCED MODELING (XGBoost/Random Forest)")
        print("="*80)
        ...
        for target in self.target_names:
            ...
            rf_grid = GridSearchCV(rf_model, rf_param_grid, cv=3, scoring='r2', n_jobs=-1, verbose=0)
            ...
            if XGBOOST_AVAILABLE:
                ...
                xgb_grid = GridSearchCV(xgb_model, xgb_param_grid, cv=3, scoring='r2', n_jobs=-1, verbose=0)
                ...
                xgb_cv_scores = cross_val_score(xgb_best, X, y, cv=5, scoring='r2')
                ...
                if xgb_test_r2 > rf_test_r2:
                    target_results['best_model'] = 'xgboost'
                    print(f"      🏆 Best model: XGBoost (R² = {xgb_test_r2:.4f})")
                else:
                    target_results['best_model'] = 'random_forest'
                    print(f"      🏆 Best model: Random Forest (R² = {rf_test_r2:.4f})")
            else:
                target_results['best_model'] = 'random_forest'
                print(f"      🏆 Using Random Forest (R² = {rf_test_r2:.4f})")
            
            # Store train/test splits for later use
            target_results['X_train'] = X_train
            target_results['X_test'] = X_test
    ...
    def visualize_advanced_results(self):
        """
        Visualize advanced modeling results with comprehensive comparisons
        """
        fig, axes = plt.subplots(3, 3, figsize=(22, 18))
        fig.suptitle('Stage 3: Advanced Modeling Results Comparison', fontsize=20, fontweight='bold')
        
        colors = ['#2E8B57', '#4169E1', '#DC143C']  # Sea Green, Royal Blue, Crimson
        model_colors = {'random_forest': '#FF6B35', 'xgboost': '#004E89', 'linear': '#A8DADC'}
        ...
        for target in self.target_names:
            ...
            performance_metrics[target]['Linear'] = linear_results['test_r2']
            
            # Advanced models
            advanced_results = self.results['stage3'][target]
            performance_metrics[target]['Random Forest'] = advanced_results['random_forest']['test_r2']
            
            if 'xgboost' in advanced_results:
                performance_metrics[target]['XGBoost'] = advanced_results['xgboost']['test_r2']
        ...
        for i, target in enumerate(self.target_names):
            ...
            bars = ax.bar(models, scores, color=[model_colors.get(m.lower().replace(' ', '_'), '#666666') for m in models],
    ...
    def stage4_deep_interpretation(self):
        """
        Stage 4: Deep Interpretation - Feature importance and PDP analysis
        """
        print("\n" + "="*80)
        print("🔬 STAGE 4: DEEP INTERPRETATION (PDP ANALYSIS)")
        print("="*80)
        ...
        for target in self.target_names:
            print(f"\n🎯 Deep interpretation for {target.replace('_', ' ')}:")
            
            target_results = self.results['stage3'][target]
            best_model_type = target_results['best_model']
            ...
            target_interpretation['pdp'] = self.generate_pdp_analysis(best_model, X, target)
    ...
    def generate_pdp_analysis(self, model, X, target):
        """
        Generate comprehensive Partial Dependence Plot analysis
        """
        # Calculate optimal subplot layout for current number of features
        n_features = len(self.feature_names)
        n_cols = 3
        n_rows = (n_features + n_cols - 1) // n_cols  # Ceiling division
        
        fig, axes = plt.subplots(n_rows, n_cols, figsize=(18, 4 * n_rows))
        fig.suptitle(f'Partial Dependence Analysis: {target.replace("_", " ")}', fontsize=16, fontweight='bold')
        ...
        for i, feature in enumerate(self.feature_names):
            ...
            pdp_result = partial_dependence(model, X, [i], grid_resolution=50)
            feature_values = pdp_result['grid_values'][0]
            pdp_values = pdp_result['average'][0]
            
            # Plot PDP
            ax.plot(feature_values, pdp_values, linewidth=3, color='#2E8B57')
            ax.fill_between(feature_values, pdp_values, alpha=0.3, color='#2E8B57')
            
            # Add rug plot (actual data points)
            feature_data = X.iloc[:, i]
            y_min = pdp_values.min()
            ax.scatter(feature_data, np.full(len(feature_data), y_min), alpha=0.3, s=10, color='red')
            ...
            pdp_results[feature] = {
                ...
                'pdp_range': (pdp_values.min(), pdp_values.max())
            }
        
        # Remove empty subplots if needed
        total_plots = n_rows * n_cols
        if n_features < total_plots:
    ...
    def generate_comprehensive_report(self):
        ...
        for target in self.target_names:
            ...
            linear_r2 = self.results['stage2'][target]['test_r2']
            advanced_results = self.results['stage3'][target]
            rf_r2 = advanced_results['random_forest']['test_r2']
            
            print(f"      Linear Regression:  R² = {linear_r2:.4f}")
            print(f"      Random Forest:      R² = {rf_r2:.4f}")
            
            if 'xgboost' in advanced_results:
                xgb_r2 = advanced_results['xgboost']['test_r2']
                print(f"      XGBoost:            R² = {xgb_r2:.4f}")
                
                best_r2 = max(linear_r2, rf_r2, xgb_r2)
            ...
            print(f"      📊 Improvement over Linear: {improvement:.4f} ({improvement/linear_r2*100:+.1f}%)")
...
if __name__ == "__main__":
    ...
    print("   3. Advanced Modeling (XGBoost/RF)")
    print("   4. Deep Interpretation (PDP)")
    print()
    ...
    try:
        ...
        print("📊 Advanced modeling has been applied with comprehensive interpretation.")
        print("🔍 Results include correlation analysis, model comparisons, feature importance, and PDP analysis.")
        print("🧪 Advanced correlation analysis conducted to address potential masking effects.")
        
    except Exception as e:
        print(f"\n❌ Analysis failed with error: {str(e)}")
        print("🔧 Please check data availability and library installations (XGBoost).")
        import traceback
        traceback.print_exc()