...
def run_island_energy_cost_analysis():
    ...
    for idx, island in cost_df.iterrows():
        ...
        # Heating Demand Total and Seasonal Variation
        heating_demand_total = demand_df['heating_demand'].sum()
        h_monthly_mean = demand_df.groupby('month')['heating_demand'].mean()
        h_monthly_mean = h_monthly_mean[h_monthly_mean > 0.1]
        heating_demand_seasonal = h_monthly_mean.max() / h_monthly_mean.min() if not h_monthly_mean.empty and h_monthly_mean.min() > 0 else 1

        # Cooling Demand Total and Seasonal Variation
        cooling_demand_total = demand_df['cooling_demand'].sum()
        c_monthly_mean = demand_df.groupby('month')['cooling_demand'].mean()
        c_monthly_mean = c_monthly_mean[c_monthly_mean > 0.1]
        cooling_demand_seasonal = c_monthly_mean.max() / c_monthly_mean.min() if not c_monthly_mean.empty and c_monthly_mean.min() > 0 else 1
        ...
        for name, col in renewables.items():
            if col in output_df.columns and not output_df[col].empty:
                ...
                # Seasonal Variation - 计算两种方法
                monthly_mean = output_df.groupby('month')[col].mean()
                monthly_mean_filtered = monthly_mean[monthly_mean > 0.01]  # 过滤掉发电量很小的月份

                # 方法1：变异系数 (Coefficient of Variation)
                if len(monthly_mean_filtered) >= 2:
                    seasonal_variation_cv = monthly_mean_filtered.std() / monthly_mean_filtered.mean()
                else:
                    seasonal_variation_cv = 0

                # 方法2：最大值/最小值比例 (Max/Min Ratio) - 使用与fig2.2.ipynb相同的resample方法
                if len(output_df) >= 240:  # 至少一个月的数据
                    # 创建时间序列，与fig2.2相同
                    start_date = '2020-01-01'
                    time_steps = len(output_df)
                    hours = pd.date_range(start=start_date, periods=time_steps, freq='3h')
                    ts = pd.Series(output_df[col].values, index=hours)

                    # 按自然月聚合，与fig2.2相同
                    monthly_means_resample = ts.resample('ME').mean()

                    # 过滤条件与fig2.2相同
                    valid_monthly_means = monthly_means_resample[monthly_means_resample > 0].dropna()

                    if len(valid_monthly_means) >= 3:  # 与fig2.2相同
                        max_monthly = valid_monthly_means.max()
                        min_monthly = valid_monthly_means.min()
                        if min_monthly > 0:
                            seasonal_variation_ratio = max_monthly / min_monthly
                        else:
                            seasonal_variation_ratio = max_monthly / 0.01  # 与fig2.2相同
                    else:
                        seasonal_variation_ratio = 1.0
                else:
                    seasonal_variation_ratio = 1.0

                # 暂时使用变异系数方法作为主要指标
                seasonal_variation = seasonal_variation_cv
                ...
                seasonal_variation_cv, seasonal_variation_ratio = 0, 1

            renewable_stats[f'{name}_total'] = total_utilization
            renewable_stats[f'{name}_seasonal'] = seasonal_variation  # 变异系数方法
            renewable_stats[f'{name}_seasonal_ratio'] = seasonal_variation_ratio  # 最大值/最小值比例方法
        ...
        island_data.append({
            ...
            'Total Wind Utilization': renewable_stats['wind_total'], 'Wind Seasonal Variation': renewable_stats['wind_seasonal'], 'Wind Seasonal Ratio': renewable_stats['wind_seasonal_ratio'], 'Wind Daily Fluctuation': renewable_stats['wind_daily'],
            'Total PV Utilization': renewable_stats['pv_total'], 'PV Seasonal Variation': renewable_stats['pv_seasonal'], 'PV Seasonal Ratio': renewable_stats['pv_seasonal_ratio'], 'PV Daily Fluctuation': renewable_stats['pv_daily'],
            'Total Wave Utilization': renewable_stats['wave_total'], 'Wave Seasonal Variation': renewable_stats['wave_seasonal'], 'Wave Seasonal Ratio': renewable_stats['wave_seasonal_ratio'], 'Wave Daily Fluctuation': renewable_stats['wave_daily'],
    ...
    wave_columns = ['Total Wave Utilization', 'Wave Seasonal Variation', 'Wave Daily Fluctuation', 'Wave Seasonal Ratio']
    ...
    independent_vars = [
        ...
        'Total Wind Utilization', 'Wind Seasonal Variation', 'Wind Seasonal Ratio', 'Wind Daily Fluctuation',
        'Total PV Utilization', 'PV Seasonal Variation', 'PV Seasonal Ratio', 'PV Daily Fluctuation',
        'Total Wave Utilization', 'Wave Seasonal Variation', 'Wave Seasonal Ratio', 'Wave Daily Fluctuation'
    ]
    ...
    variability_vars = [
        ...
        'Wind Seasonal Variation', 'Wind Seasonal Ratio', 'PV Seasonal Variation', 'PV Seasonal Ratio',
        'Wave Seasonal Variation', 'Wave Seasonal Ratio'
    ]
    ...
    for i, y_var in enumerate(dependent_vars):
        for j, x_var in enumerate(independent_vars):
            ...
            if x_var in variability_vars:
                # 针对不同可再生能源类型和方法使用不同的筛选策略
                if 'Wave' in x_var:
                    # 波浪能：需要有显著的总利用量和变异性才纳入分析
                    wave_utilization_condition = final_df['Total Wave Utilization'] > 100  # 有一定的波浪能利用
                    if 'Ratio' in x_var:
                        # 比例方法：阈值设为大于1.5（与fig2.2.ipynb一致）
                        wave_variability_condition = final_df[x_var] > 1.5
                    ...
                    # 其他可再生能源：根据方法类型使用不同阈值
                    if 'Ratio' in x_var:
                        # 比例方法：阈值设为大于1.5
                        current_df = final_df[final_df[x_var] > 2].copy()
                        min_threshold_text = 'ratio>1.5'
                    else:
                        # 变异系数方法：阈值设为大于0.01
    ...
    plt.savefig('regression/regression_subplots.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig('regression/r2_pvalue_matrix.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.savefig('regression/multicollinearity_analysis.png', dpi=300, bbox_inches='tight')
    plt.close()
